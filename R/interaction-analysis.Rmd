...
graph_1_nuit <- graph_1  %>%
  filter(hour(graph_1$date_posix) <= 5) %>%
  filter(length >= 300)
```
...
for (i in 1:24){
  temp <- graph_1  %>% filter(hour(graph_1$date_posix) <=  5) %>%
  filter (length >= 60 * 5 * i) %>%
  group_by(date(date_posix)) %>%
  summarise(count = n(), count_distinct = length(unique(c(from, to)))) %>%
  summarise(min_count = min(count), max_count = max(count) ,min_distinct = min(count_distinct), max_distinct = max(count_distinct))
...
graph_1_bis <- graph_1  %>% filter(hour(graph_1$date_posix) <=  5) %>%
  filter (length >= 600) %>%
...
graph_1  %>% filter(hour(graph_1$date_posix) <=  5) %>%
  ...
  group_by(date(date_posix)) %>% mutate(localisation = case_when((gsub("-.*", "", from) == "PA" & gsub("-.*", "", to) == "PE") ~ "chambrePA-from",
                                                                 ...
                                                                 (gsub("-.*", "", from) == "PE" & gsub("-.*", "", to) == "PE") ~ "restroom")) %>%
  mutate(from_n = pmin(from,to), to_n = pmax(from, to)) %>% 
  select(-c("to","from")) %>% ungroup() %>% group_by(localisation) %>% summarise()
