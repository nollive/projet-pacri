...
# Prepare reconstructed data 
for (prob in c("0", "0-1", "0-3", "0-5")) {
  dur_sim = data.frame()
  distrib_simu = data.frame()
  
  for (f in list.files(file.path(synthetic_path, paste0("full_", prob)), "matContactBuilt", full.names = T)) {
   data = read.csv2(f)
...
for (prob in c("0", "0-1", "0-3")) {
  ...
  simu_files = list.files(path = file.path(synthetic_path, paste0("full_", prob)), pattern = "matContact.*", full.names = T)
