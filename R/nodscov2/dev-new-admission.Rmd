...
truncated_data <- lapply(1:length(truncated_data), function(t){
  df <- truncated_data[[t]] %>% mutate(time = t)
  return(df)
})
...
df_data_long <- do.call(rbind, test_data)
df_interaction_long <- do.call(rbind, test_interaction)
...
for (patient in id_patient){
  ...
  while (i < new_n_subdivisions & iter < 25) {
    ...
    df_data_long <- df_data_long %>% 
      mutate(id = ifelse(id == patient & between(time,i,i + i_sampled - 1), new_id, id))
    df_interaction_long <- df_interaction_long %>%
      mutate(from = ifelse(from == patient & between(time,i,i + i_sampled - 1) , new_id, from),
             to = ifelse(to == patient, new_id, to))
...
global_data <- lapply(1:new_n_subdivisions, function(t) {
  data_t <- df_data_long %>% filter(time == t) %>% arrange(id)
  return(data_t)
})

global_interaction <- lapply(1:new_n_subdivisions, function(t) {
  interaction_t <- df_interaction_long %>% filter(time == t)
  return(interaction_t)
})
