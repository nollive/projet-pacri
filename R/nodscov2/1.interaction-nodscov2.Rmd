...
# Get number of overlapping contacts
verify_pair_distribution %>%
  distinct(from, to, pair) %>%
  group_by(pair) %>%
  mutate(n = n()) %>%
...
#   mutate(
#     length = case_when(
#       date_posix_first <= noon_day1 ~ as.numeric(difftime(date_posix_first+length, noon_day1, units = "sec")),
#       date_posix_first+length >= noon_day2 ~ as.numeric(difftime(noon_day2, date_posix_first, units = "sec")),
#       .default = length 
#     ),
