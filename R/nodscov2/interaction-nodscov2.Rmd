...
# 001-0127-B-A - 
# max(k_RP$date_posix_first) - min(k_RP$date_posix_first)
# max(k_RP$date_posix_first) - min(k_RP$date_posix_first) - (109770 / 3600 / 24)



k_RP %>% filter(from %in% unique_ids | to %in% unique_ids) %>% filter(from %in% b | to %in% b) %>% 
## "001-0038-B-S" and "001-0039-B-F" are in the same room (HCW interaction with both at the same time, not interacting all the time maybe because CPI too distant)
## "001-0127-B-A" and "001-0128-Z-K" are in the same room (109770s = 30.49167 hour out of 39.78888hours)
...
pe_individuals <- unique(c(data_b$to[data_b$to_status == "PE"]))
b <- c("001-0038-B-S", "001-0039-B-F")
simultaneous_interactions <- list()


for (individual in pe_individuals) {
    ...
    for (i in 1:(nrow(interactions)-1)) {
      for (j in (i+1):nrow(interactions)) {
        ...
        if (interactions$date_posix_first[i] <= (interactions$date_posix_first[j] + interactions$length[j]) &&
          ...
          # Print the int (store it?)
          cat("Individu:", individual, "\n")
          print(interactions[c(i, j), ])
          cat("\n")
        }
      }
    }
...
```
```{r}
data <- list_rea[[herriot]]
begin_date <- as.POSIXct(min(data$date_posix_first)) - 10 ## to begin at "2020-05-06 07:00:00"
end_date <- as.POSIXct(max(data$date_posix_first + data$length))
time_spent <- end_date - begin_date
...
lin_b <- Sys.time()
print(lin_b-lin_a)
write.csv2(interactions_tot_R, file = file.path(wd,"..", "..", "data", "data-nodscov2", "int_tot_EH_test.csv"), row.names = F)
