...
```{r}
# admission_keep %>% filter(hospital != "APHP - RAYMOND POINCARÉ") %>% group_by(status) %>% summarise(count = n())
# 
# admission_keep %>% filter(hospital == "APHP - RAYMOND POINCARÉ") %>% group_by(status) %>% summarise(count = n())
...
temp_RP <- list_rea[[poincare]] #%>% filter(length > 300)

temp_EH <- list_rea[[herriot]] #%>% filter(length > 300)
