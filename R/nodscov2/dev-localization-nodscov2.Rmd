...
global_localization <- lapply(1:n_subdivisions, function(t) {
  ...
  lapply(clusters_t, function(cluster) {
    ...
    if (MULTIPLE_patients_in_cluster){
      ...
      cluster_localization <- rooms[rooms$id == "M-PM", "id_room"] ## ONLY MEDICAL AND PARAMEDICAL -> ???
    } else {
...
## Store info when there is more than one patient in clusters -> fix later
cluster_info_list <- list()
global_localization_bis <- list()


global_localization_bis <- lapply(1:n_subdivisions, function(t) {
  localization_t <- data.frame(id = HCW_interacting_ids, localization = NA, time = t) ##only hcw that are actually here 
  ...
  lapply(clusters_t, function(cluster) {
    ...
    ## localization OF THE CLUSTER
    if (MULTIPLE_patients_in_cluster){
      cluster_localization <- rooms_bis[rooms_bis$id == "ALL", "id_room"]
    } else if (IN_DOUBLE_ROOM_N1) {
      cluster_localization <- unique(rooms_bis[rooms_bis$id == double_rooms_bis[[1]][[1]], "id_room"])
    } else if (IN_DOUBLE_ROOM_N2) {
      cluster_localization <- unique(rooms_bis[rooms_bis$id == double_rooms_bis[[2]][[1]], "id_room"])
    } else if (ONE_patient_in_cluster) {
      cluster_localization <- rooms_bis[rooms_bis$id == cluster[!cluster %in% HCW_interacting_ids], "id_room"]
    } else {
      cluster_localization <- rooms_bis[rooms_bis$id == "PE", "id_room"] ## ONLY HCW INTERACTING
    }
    ## Assign the localization for individuals inside the cluster (<<- to edit localization_t defined outside clusters_t's lapply scope)
    localization_t$localization <<- ifelse(localization_t$id %in% cluster, cluster_localization, localization_t$localization)
    ...
    ## MULTIPLE PATIENTS IN THE CLUSTER --> TEMPORARY, FIXED LATER
    if (MULTIPLE_patients_in_cluster) {
      cluster_possible_localization <- rooms_bis[rooms_bis$id %in% cluster[!cluster %in% HCW_interacting_ids], "id_room"]
      cluster_info <- data.frame(Cluster = t, Patients = cluster[!cluster %in% HCW_interacting_ids])
      cluster_info_list <<- append(cluster_info_list, list(cluster_info))
    }
  })
...
rooms_bis <- rooms_bis %>%
  bind_rows(data.frame(room = "Restroom", id = "PE", id_room = as.factor(nrow(rooms_bis) + 1))) %>%
  bind_rows(data.frame(room = "Corridor", id = "ALL", id_room = as.factor(nrow(rooms_bis) + 2)))

### DOUBLE ROOMS
## "001-0038-B-S" and = "001-0039-B-F" are in the same room
## "001-0127-B-A" and "001-0128-Z-K" are in the same room
rooms_bis <-rooms_bis %>% mutate(room = ifelse(id == "001-0038-B-S", rooms_bis[rooms_bis$id == "001-0039-B-F", "room"], room),
                         id_room = ifelse(id == "001-0038-B-S", rooms_bis[rooms_bis$id == "001-0039-B-F", "id_room"], id_room))

rooms_bis <-rooms_bis %>% mutate(room = ifelse(id == "001-0128-Z-K", rooms_bis[rooms_bis$id == "001-0127-B-A", "room"], room),
                         id_room = ifelse(id == "001-0128-Z-K", rooms_bis[rooms_bis$id == "001-0127-B-A", "id_room"], id_room))

double_rooms_bis <- list(c("001-0038-B-S", "001-0039-B-F"), c("001-0127-B-A", "001-0128-Z-K"))


test_loc_bis <-  do.call(rbind, global_localization_bis) %>% left_join(rooms_bis %>% select(id_room, room), by = c("localization" = "id_room"))
View(test_loc_bis %>% group_by(localization) %>% summarise (n = n()) %>% left_join(rooms_bis %>% select(id_room, room), by = c("localization" = "id_room")))
```
