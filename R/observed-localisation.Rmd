...
for (ind in id_individual %>% filter(substr(id,1,2) == "PA") %>% pull(id)){
  ...
  for (adm in seq_len(admission %>% filter(id == ind) %>% nrow())) {
    ...
    for (ind_adm in unique_ind_adm[which(unique_ind_adm != ind)]){
      ...
      ## WE MUST PUT THE ID OF IND AT FROM & ID OF IND_ADM IN TO (group_by will not work as there is from:id1, to:id2 and from:id2, to:id1)
...
```{r}
# Function to perform Stable Marriage with Incomplete Lists
stable_marriage_incomplete <- function(rankings_list) {
    # Extracting unique individuals
    individuals <- unique(unlist(lapply(rankings_list, `[[`, "other_individual")))
    
    # Creating preference matrices for both sets of individuals
    prefs1 <- sapply(individuals, function(individual) {
        prefs <- unlist(lapply(rankings_list, function(df) {
            df[df$other_individual == individual, "total_time_with_individual"]
        }))
        names(prefs) <- unlist(lapply(rankings_list, function(df) {
            df[df$other_individual == individual, "other_individual"]
        }))
        return(prefs)
    })
    ...
    # Applying the SMI algorithm
    matches <- matrix(NA, nrow = length(individuals), ncol = 2)
    for (i in 1:length(individuals)) {
        current_individual <- individuals[i]
        prefs <- prefs1[[i]]
        sorted_prefs <- sort(prefs, decreasing = FALSE)
        for (j in 1:length(sorted_prefs)) {
            current_partner <- names(sorted_prefs)[j]
            if (is.na(matches[i, 2])) {
                matches[i, 1] <- current_individual
                matches[i, 2] <- current_partner
            } else {
                prefs_partner <- prefs2[[which(individuals == current_partner)]]
                if (current_individual %in% names(prefs_partner)) {
                    if (prefs_partner[current_individual] < prefs_partner[matches[i, 1]]) {
                        matches[i, 1] <- current_individual
                        matches[i, 2] <- current_partner
                    }
                }
...
# Applying the SMI algorithm to the provided data
matches <- stable_marriage_incomplete(ranking_list)
print(matches)
