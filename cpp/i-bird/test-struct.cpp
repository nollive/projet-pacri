...
status Update_status(
  ...
  lambda_c lambda_c_ti = Lambda_c(lambda_c_tim1, interaction_ti, status_ti, beta, dt, ti);
  lambda_e lambda_e_ti = Lambda_e(lambda_e_tim1, localisation_ti, environment_ti, epsilon, dt);
  for (int j = 0; j < status_tim1.individual.size(); j++){
    if (status_tim1.status[j] == 0){
      if (((double)rand()) / RAND_MAX < (1 - exp(-lambda_c_ti.lambda[j] - lambda_e_ti.lambda[j]) )){
        status_tim1.status[j] == 1;
      }
    }
  }
